...
Tessera is a production-ready, self-hosted AI agent platform built around the principle that security cannot be bolted on later. Every tool call is sandboxed, every credential stays in a vault, every decision is audited, and the agent can never be jailbroken into acting outside your explicitly defined policy.

---

## Why Tessera?

| Concern | How Tessera addresses it |
|---|---|
...
All configuration is via environment variables. Run `tessera init` to generate a `.env` file automatically. See [`.env.example`](.env.example) for the full reference.
...
|---|---|
| `GATEWAY_HMAC_SECRET` | Secret for signing auth tokens. 64 hex chars (generated by `tessera init`). |
...
| `TESSERA_ALLOW_RUNC` | `false` | Dev escape hatch — disables gVisor check. **Never use in production.** |
...
agent.session → gen_ai.chat → gen_ai.usage.*
             → tessera.tool.run → tessera.tool.*
             → tessera.approval.wait → tessera.approval.*
```
...
```bash
tessera init                                    # First-run wizard: generate secrets, write .env
tessera token generate --user <userId>          # Generate a bearer token
tessera health [--url <url>]                    # Check gateway health
tessera session create [--provider anthropic]   # Start an agent session
tessera session status <sessionId>              # Get session status
tessera session delete <sessionId>              # Terminate a session
tessera skill list     [--search <q>]           # Browse the marketplace
tessera skill publish  <manifest.json> [--trivy] # Publish a skill
tessera skill install  <ns/name[@ver]>          # Install from marketplace
tessera skill installed                         # List installed skills
```
...
tessera skill publish manifest.json --trivy
```
...
│   ├── cli/               # `tessera` CLI (init, token, session, skill, health)
│   └── channels/
...
- [x] First-run setup wizard (`tessera init`)
- [x] Cross-platform CI matrix (Windows × macOS × Linux × Node 20/22)
- [x] Configurable token expiry + refresh endpoint (`TOKEN_EXPIRY_SECONDS`, `tessera token refresh`)

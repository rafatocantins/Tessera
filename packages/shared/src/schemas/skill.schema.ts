...
export const SkillToolDefinitionSchema = z.object({
  ...
  image: z.object({
    /** Registry + repository, e.g. "docker.io/secureclaw/file-read" */
...
export const SkillManifestSchema = z.object({
   ...
   * Generated by `generateEd25519KeyPair()` from @secureclaw/shared.
  ...
  /**
   * Minimum SecureClaw version required to run this skill.
   * Skills engine will reject installs if the running version is lower.
   */
  min_secureclaw_version: z
    .string()
    .regex(/^\d+\.\d+\.\d+$/)
    .optional(),
});

/**
 * sandbox.client.ts — Thin gRPC client for SandboxService used by skills-engine.
 *
...
export class SandboxGrpcClient {
  ...
  constructor(addr?: string) {
    ...
    this.client = new SandboxServiceClient(target, clientCredentials("skills-engine"));
  }
  ...
  runTool(params: RunToolParams): Promise<RunToolResult> {
    return new Promise((resolve, reject) => {
      const req: GrpcRunToolRequest = {
        ...
        limits: {
          memory_bytes: params.memory_bytes ?? 64 * 1024 * 1024,
          cpu_shares: 0.5,
          pids_limit: params.pids_limit ?? 16,
        },
